...
EventController.tBetweenEventTimeRange = { 100, 240 }
...
function EventController.setBaseSeeds()
    ...
    local tStartingData = MiscUtil.getGalaxyMapValues(tLandingZone.x, tLandingZone.y)
    ...
    -- if total avg frequency between immigration and derelicts and hostility > 1, make events more frequent.
    EventController.tS.nTimeBetweenEventsMod = math.min( math.max(1.0 / nAvg, .8), 1.0)
end
...
function EventController.fromSaveData(tSaveData)
    ...
    print('time between events mod',EventController.tS.nTimeBetweenEventsMod)
end
...
function EventController._getNextEventTimeDelta()
    ...
    local t = EventController.tBetweenEventTimeRange[1] * alpha + 
              EventController.tBetweenEventTimeRange[2] * (1.0 - alpha) + 
              math.random(-30, 30)
    return t
end

function EventController.generateEventForecast()
    ...
    local nPopulationDeltaEstimate = 0
    local nTimeDeltaEstimate = 0
    ...
    for i=1,EventController.nEventForecastMax do
        ...
        local nPopulationEstimate = CharacterManager.getOwnedCitizenPopulation() + nPopulationDeltaEstimate
            ...
            if tLocs[sLocName] then
                    ...
                    elseif sDataType == "objects" then
                        ...
                        if spawnData.bInvItem then
                            ...
                            local tData, bPickup = EnvObject.getObjectData(spawnData.sType)
                            if tData then
                                local nTeam = fnGetTeam(spawnData,tTeams)
                                if bPickup then
                                    Pickup.createPickupAt(spawnData.sType, wx,wy, spawnData.tSaveData, nTeam)
                                else
                                    EnvObject.createEnvObject(spawnData.sType, wx, wy, spawnData.bFlipX, spawnData.bFlipY, true, spawnData.tSaveData, nTeam)
                                end
                            else
