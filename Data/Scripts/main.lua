...
local function _initGame(windowWidth, windowHeight)
    ...
    --DataCache.createCache("compoundtex", 50)
    --DataCache.createCache("geo", 50)
    --DataCache.createCache("dialogset", 10)
    --DataCache.createCache("linecode", 10)
    ...
    DataCache.createCache("linecode", 100)
...
function main()
    ...
    -- XXX TODO: remove this code by alpha 6 -bpeck XXX
    -- Migrate linux saves from old location to new
    if MOAIEnvironment.osBrand == "Linux" then
        local sHomedir = os.getenv('HOME')
        if sHomedir and #sHomedir > 0 then
            local sOldSavePath = sHomedir .. '/.config/spacebasedf9/Saves/SpacebaseDF9AutoSave.sav'
            local sNewSavePath = MOAIEnvironment.documentDirectory .. '/Saves/' .. GameRules.sAutoSaveFile .. '.sav'
            if not MOAIFileSystem.checkFileExists(sNewSavePath) and MOAIFileSystem.checkFileExists(sOldSavePath) then
                print('LINUX USER, TAKE NOTE: Copying old save game from ' .. sOldSavePath .. ' to ' .. sNewSavePath)
                -- We don't have a new save game, and we DO have an old one. copy the old one over
                MOAIFileSystem.copy(sOldSavePath, sNewSavePath)
            end
        end
    end
