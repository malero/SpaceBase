...
-- Calculate a modifier for this event type based on the base's spawn location
-- in the galaxy. This gets stored in the save file, so it's kinda-static since
-- it varies, but on a per-base basis. base basis base basis base basis
function Event.getSpawnLocationModifier(nMin, nMax, sSpawnLocModifierKey)
    nMin = nMin or 0.0
    nMax = nMax or 1.0
    local tLandingZone = GameRules.getLandingZone()
    local nSpawnLocModifier = MiscUtil.getGalaxyMapValues(tLandingZone.x,tLandingZone.y)[sSpawnLocModifierKey] or math.random()
    return DFMath.lerp(nMin, nMax, nSpawnLocModifier)
end

function Event.getPopulationMod()
    return Event.getSpawnLocationModifier(0.6, 1.7, 'population')
end

function Event.getHostilityMod(bHostile)
    local nMod = Event.getSpawnLocationModifier(0.6, 1.7, 'hostility')
    if not bHostile then return 1/nMod end
    return nMod
end
