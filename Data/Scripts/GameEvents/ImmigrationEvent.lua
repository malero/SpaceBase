...
function ImmigrationEvent.getWeight(nPopulation, nElapsedTime)
    ...
    if CharacterManager.getOwnedCitizenPopulation() >= g_nPopulationCap then
        return 0
    end
	-- in early game try to help player get to a minimum viable population size
	if nElapsedTime < ImmigrationEvent.EARLY_POPULATION_TIME and CharacterManager.getOwnedCitizenPopulation() < ImmigrationEvent.EARLY_POPULATION_THRESHOLD then
		return 75.0
	end
    return 50.0
end
...
function ImmigrationEvent.preAlertSetup(rController, tUpcomingEventPersistentState)
    if not tUpcomingEventPersistentState.bHostile and CharacterManager.getOwnedCitizenPopulation() >= g_nPopulationCap then
        return false
    end
    return true
end
