...
function EnvObject.getSpriteLoc(propName,wx,wy,bFlipX,bFlipY)
    ...
    if bFlipX then
        ...
        local spriteSheetPath = (bPickup and require('Pickups.Pickup').spriteSheetPath) or EnvObject.spriteSheetPath
...
function EnvObject.createBuildGhost(propName, tx, ty, bFlipX, bFlipY)
    ...
    if bPickup then
        ...
        spriteSheet = DFGraphics.loadSpriteSheet(EnvObject.spriteSheetPath, false, false, false)
    end
    local spriteName = tData.spriteName
...
function EnvObject.createEnvObject(propName, wx, wy, bFlipX, bFlipY, bForce, tSaveData, nTeam)
    ...
    if bFlipX and not EnvObject.canFlipX(propName) then
        Print(TT_Warning, "Prop not allowed to be flipped. Overriding.",propName)
        bFlipX = false
    end
    ...
    if bForce or EnvObject.allowObjInRoom(tData,rRoom) then
        ...
        if bFits or bForce then
            ...
            Print(TT_Warning,'Failed to build prop.',propName,wx,wy)
            return
        end
    end
end
...
function EnvObject:init(sName, wx, wy, bFlipX, bFlipY, bForce, tSaveData, nTeam)
    ...
    self.spriteSheet = DFGraphics.loadSpriteSheet( self.spriteSheetPath )
...
function EnvObject:onInteract(bStart,rChar)
    ...
    if bStart and self.rUser then
        Print(TT_Error, "New user A clobbering old user B",rChar.tStats.sUniqueID,self.rUser.tStats.sUniqueID)
    end
...
function EnvObject:isFunctioning()
	return self:hasPower() and self.nCondition > 0 and not self.bDestroyed
end
