...
function Gui._popPaneStack()
    if #Gui.paneStack > 0 then
        ...
        print("popped pane stack. Show:", Gui.rCurActivePane.dbgName or "(no debug name)")
...
function Gui.setActivePane(rPane, bPush, nBasePri)
	...
	if not bPush and Gui.rCurActivePane ~= nil then
        if Gui.rCurActivePane.dbgName then print("Hide",Gui.rCurActivePane.dbgName) end
    ...
    if bPush then
        if Gui.rCurActivePane then
            ...
            Print(TT_Warning, "Attempting to push onto gui stack while no gui element is showing.")
        end
    end
    ...
    local pri = 0
    if Gui.rCurActivePane then
        if Gui.rCurActivePane.dbgName then print("Show",Gui.rCurActivePane.dbgName, "push?",bPush) end
