...
--                    return wy < y+h and wy > y
    ...
    function Ob:setVisible(bVisible)
        if self.rParentUIElement then
            self.rParentUIElement:setElementHidden(self, not bVisible)
        else
        ...
        if tInfos and self.tTemplateElements then
            for i, tElementInfo in ipairs(tInfos) do
                ...
                if rElement then
                    ...
                    --[[
                elseif tElementInfo.playSfx then
                    if g_GuiManager.bFinishedInit then
                        SoundManager.playSfx(tElementInfo.playSfx)
                    end
                    ]]--
                end
            end
...
--        local tElems = tElemListOverride or self.tTemplateElements
    ...
    function Ob:onResize()
        self:alignToPosInfo()
        --[[
        if self.tSavedElementInfo then
            self:_applyResizeChanges(self, self.tSavedElementInfo)
        end
        for _, v in ipairs(self.tElements) do
            if v.onResize then 
                v:onResize()
