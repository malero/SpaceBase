...
function GameRules.checkCameraPan(dt)
    ...
    if rKeyboard:keyIsDown(string.byte("w")) then    
        dy = dy + 1
    end
    
    if rKeyboard:keyIsDown(string.byte("s")) then        
        dy = dy - 1       
    end
    
    if rKeyboard:keyIsDown(string.byte("a")) then        
        dx = dx - 1
    end
    
    if rKeyboard:keyIsDown(string.byte("d")) then
        dx = dx + 1
    end
...
function GameRules.randomSetup(tLandingZone)
    ...
    require('Asteroid').spawnAsteroids(tStartingData.asteroids)
    EventController.setBaseSeeds()
...
function GameRules.deleteCharacter(wx,wy)
    ...
    if rProp.tag.objType == ObjectList.CHARACTER then
