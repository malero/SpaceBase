...
-- following number is for UI display, real # derived from EnvObjectData
GoalData.TARGET_ENVOBJECT_TYPES = 34
GoalData.TARGET_MEALS = 1000
GoalData.TARGET_CURES = 10
GoalData.TARGET_TECHS = 13 -- derived from ResearchData
...
function GoalData.finalSiege()
	...
	local tCitizens = CharacterManager.getOwnedCharacters()
	local bFriendlySurvivor = false
	for _,rChar in pairs(tCitizens) do
		if not Malady.isIncapacitated(rChar) then
